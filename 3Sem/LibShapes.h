...
class Star: public Figure
{
    private:
        ...
        type d;   // Длина основания луча (сторона внутреннего пятиугольника)
                  // The length of the base of the beam (side of the inner pentagon)
    public:
        ...
        virtual type Calc_Square()
        {
            ...
            double c = Fi * d;           // Ребро луча (Beam edge)

            double p = (d+c+c) / 2;   // Полупериметр (Half-meter)

            double AreaTriangle = sqrt( p * (p-c)*(p-c)*(p-d) );   // Формула Герона (Heron 's Formula)
